...
async function fetchWithConfig(url, userAgent) {
...
function analyzeHeaders(headers) {
  const findings = { cloudflare: false, cloudfront: false, waf: false, reasons: [], signals: {} };
  const h = {};
...
function compareUaResponses(browserReq, botReq) {
  const findings = { botDetection: false, reasons: [], signals: {} };

  const lenDiff = Math.abs(browserReq.html_length - botReq.html_length);
  const linkDiff = Math.abs(browserReq.link_count - botReq.link_count);
  const lenRatio = browserReq.html_length > 0 ? lenDiff / browserReq.html_length : 0;

  findings.signals = {
    browser_html_length: browserReq.html_length,
    bot_html_length: botReq.html_length,
    length_difference: lenDiff,
    browser_link_count: browserReq.link_count,
    bot_link_count: botReq.link_count,
  ...
  if (lenRatio > 0.2 && lenDiff > 500) {
    ...
    findings.reasons.push(
      `HTML length differs significantly between browser UA (${browserReq.html_length}B) and bot UA (${botReq.html_length}B) — ${Math.round(lenRatio * 100)}% difference`
    );
  }
  if (linkDiff >= 3 && botReq.link_count < browserReq.link_count) {
    findings.botDetection = true;
    findings.reasons.push(
      `Link count differs: browser UA got ${browserReq.link_count} links, bot UA got ${botReq.link_count} links`
    );
  }
  if (botReq.status !== browserReq.status) {
    findings.botDetection = true;
    findings.reasons.push(
      `Different HTTP status codes: browser UA got ${browserReq.status}, bot UA got ${botReq.status}`
    );
  }
...
exports.handler = async (event) => {
  ...
  try {
    browserRes = await fetchWithConfig(url, BROWSER_UA);
    ...
    if (headerFindings.cloudflare) { result.cloudflare = true; score += 40; }
    if (headerFindings.cloudfront) { result.cloudfront = true; }
    if (headerFindings.waf || headerFindings.cloudfront) { result.waf = true; score += 30; }
    ...
    if (htmlFindings.suspicious || htmlFindings.blocked) score += 20;
    if (htmlFindings.cloudflareChallenge) { result.cloudflare = true; score += 20; }

    result.raw_signals.redirects = { count: browserRes.redirectCount, final_url: browserRes.finalUrl };
  ...
  try {
    const botRes = await fetchWithConfig(url, BOT_UA);
    ...
    if (browserRes) {
      ...
      if (uaFindings.botDetection) { result.bot_detection = true; score += 30; }
      result.reasons.push(...uaFindings.reasons);
      result.raw_signals.ua_comparison = uaFindings.signals;
    }

    if ((botRes.status === 403 || botRes.status === 503) && browserRes && browserRes.status === 200) {
  ...
  if (result.reasons.length === 0) result.reasons.push('No protection signals detected');
