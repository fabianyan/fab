...
function compareUaResponses(browserResult, botResult) {
  ...
  const lenRatio = browserResult.html_length > 0 ? lenDiff / browserResult.html_length : 0;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (browserRes) {
      const uaFindings = compareUaResponses(result.raw_signals.browser_request, result.raw_signals.bot_request);
